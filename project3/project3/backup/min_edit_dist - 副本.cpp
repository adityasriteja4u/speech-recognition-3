...
void outputEachTrellis(const int &templen, const int &inputlen, const int &k)
{
	for (int j = rowindex[k] + templen; j >= rowindex[k]; --j) {
		if (j != rowindex[k]) printf("%c\t", templates[k][j - rowindex[k] - 1]);
		else printf("0\t");
...
void outputBestPath(const int &templen, const int &inputlen, const int &k)
{
	for (int i = 0; i <= inputlen; ++i)
		arrow[i][0] = 0;
	for (int j = rowindex[k]; j <= rowindex[k] + templen; ++j)
		arrow[0][j] = 1;

	int path[max_y], pathlen = 0;
	int i = inputlen, j = rowindex[k] + templen;
	while (i != 0 || j != 0) {
...
void outputTrellisAndBestPath(const MatchData &data)
{
	...
	printf("=== Best Path ===\n");
	outputBestPath(data.best_match.size(), inputlen, data.best_index);
}
...
void updateColForTempK(const int &i, const int &k, const int &cur_min_cost, int &next_min_cost, MatchData &match_data)
{
	...
	if (i == inputstr.size() - 1 && dp[i + 1][rowindex[k + 1] - 1] != -1 && dp[i + 1][rowindex[k + 1] - 1] < match_data.cost)
	{
		...
		match_data.best_match = templates[k];
	}
}
...
MatchData minEditDistPrune()
{
	...
	for (int i = 0; i < inputlen; ++i) {
		...
		if (prune_type == 1) cur_min_cost = 0;
